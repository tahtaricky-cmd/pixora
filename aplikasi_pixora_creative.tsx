...
import { ImageIcon, Wand2, Download, Upload, X, Loader2, Eye, History, Sparkles, ImagePlus, RefreshCcw, Megaphone, Type, LogOut, Mail, Lock, Crown, CreditCard, ArrowLeft, AlertTriangle } from 'lucide-react';
...
export default function App() {
  ...
  const handleGenerate = async (e) => {
    ...
    try {
      ...
      const autoPropsInstruction = "Critically analyze the reference product. Automatically add highly relevant, natural-looking contextual props that perfectly match the product's category (e.g., scattered coffee beans/leaves for coffee packaging, an aesthetic hanger or folded fabrics for fashion, fresh natural ingredients for food or skincare). Ensure these automatic props enhance the scene organically without overpowering the main product.";
  ...
  const handleGeneratePromo = async () => {
    ...
    try {
      ...
      const promoScenarios = [
        "Focus: Main thumbnail image optimized for a marketplace listing. Eye-catching, highly visible product, with a clear and bold main title.",
        "Focus: Explain the product usage scenario or lifestyle context. Show or describe how it's used and its benefits.",
        "Focus: Highlight specific details, ingredients, or key features of the product using elegant text callouts or descriptions.",
        "Focus: Detailed product breakdown or specifications (rincian produk) arranged in a clean, structured, and informative layout."
      ];


      const promoPromises = promoScenarios.map(async (scenario) => {
        const promptText = `Transform this product photo into a high-quality professional promotional poster or social media ad.
        ${scenario}
        You MUST intelligently incorporate the following product information/details into the design beautifully, logically, and legibly as promotional text:
        ${detailsText}
        Overall design style: ${promoStyle}.
        STRICT NEGATIVE CONSTRAINT: Do NOT include any 'buy now', 'shop now', 'checkout', 'add to cart', or any direct transactional sales call-to-action buttons or text.
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <select
                      ...
                      className="w-full p-4 bg-slate-50 rounded-2xl border-2 border-transparent focus:border-fuchsia-300 focus:bg-white focus:ring-4 focus:ring-fuchsia-100 outline-none transition-all appearance-none font-medium"
                    >
